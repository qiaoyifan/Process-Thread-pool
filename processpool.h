/*************************************************************************
	...
	> Author:qiaoyifan 
	> Mail: qyf116126@163.com
	> Created Time: 2017年09月23日 星期六 19时29分09秒
 ************************************************************************/
...
{
private:
    /*将构造函数定义为私有的，因此我们只能通过
     * 后面的create静态函数来创建processpool实例
     */
    processpool(int listenfd, int process_number = 8);
public:
    /*单体模式，以保证程序最多创建一个processpoll实例
      这是程序正确处理信号的必要条件*/
 ...
 * 该信号默认结束进程，为了避免进程退出, 可以捕获SIGPIPE信号, 或者忽略它,
 * 给它设置SIG_IGN信号处理函数
 */
}

/* 父进程中m_idx值为-1，子进程中m_idx值大于0，我们据此判断接下来要执行的
 * 是父进程代码还是子进程代码
 */
template< typename T >
...
void processpool< T >::run_child()
{
    ...
    /*子进程需要监听管道描述符pipefd,父进程将通过它来通知子进程accept新连接*/
        ...
        for(int i = 0; i < number; ++i)
        {
            ...
            if ( (sockfd == pipefd)  && (events[i].events & EPOLLIN) )
            {
                ...
                else
                {
                    ...
                    /*模板类T必须实现init方法，以初始化一个客户连接，我们使用
                    * connfd来索引逻辑处理对象(T类型的对象),以提高程序效率*/
                        ...
                        switch(signals[i])
                        {
                            case SIGCHLD:
                            {
                                ...
                                while( (pid = waitpid(-1, &stat, WNOHANG)) > 0 )
                                {
                                    ...
                                    {
                                    /*如果进程池中第i个子进程退出了，则主进程关闭相
                                     * 应的通信管道，并设置相应的m_pid为1，以标记该
                                     * 子进程已经退出
                                     */ 
